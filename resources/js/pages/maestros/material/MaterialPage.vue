      ...
      <v-row dense class="pa-2 align-center">
        ...
        <v-col class="text-right">
          <v-btn color="primary" class="mr-2" @click="addDialog = true">
            Añadir Material <v-icon>mdi-plus</v-icon>
          </v-btn>
        </v-col>
      </v-row>
      <v-row dense class="pa-2 align-center">
        ...
        <v-card-text>
          ...
          >
            <v-row>
              <v-col>
                <v-text-field
                  v-model="addForm.nombre_material"
                  label="Nombre"
                ...
                <v-form ref="viewForm">
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <v-text-field
                                v-model="viewForm.nombre_material"
                                label="Nombre"
                                readonly
                            ></v-text-field>
                        </v-col>
...
export default {
  data() {
    return {
      ...
      headers: [
        { text: "#", value: "id_material", align: "left" },
        { text: "Material", value: "nombre_material", align: "center" },
  ...
  methods: {
    async getRegistros(page = 1, per_page = 25, sortDesc = 0, sortBy = "") {
      ...
      this.data_reg = [];
      try {
        const response = await API.producto_material.list(
          "?page=" +
            page +
            "&itemsPerPage=" +
            per_page +
    ...
    editReg(item) {
      this.addFormTitle = "Editar Material";
    ...
    async deleteRegConfirm() {
      ...
      try {
        const response = await API.producto_material.remove(
          this.addForm.id_material
        );
        UTILS.toastr.success("Material eliminado correctamente", this);
      ...
      if (this.$refs.addForm.validate()) {
        try {
          ...
          if (this.dialogEditar == false) {
            const response = await API.producto_material.create(this.addForm);
            UTILS.toastr.success("Material creado correctamente", this);
          } else {
            const response = await API.producto_material.update(
              this.addForm.id_material,
              this.addForm
            );
            UTILS.toastr.success("Material actualizado correctamente", this);
          }
          this.getRegistros();
  ...
  watch: {
    ...
    addDialog() {
      if (!this.addDialog) {
        ...
        this.addFormTitle = "Agregar Material";
      }
    },
    viewDialog() {
        if (!this.viewDialog) {
