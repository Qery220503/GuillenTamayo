      ...
      <v-row dense class="pa-2 align-center">
        ...
        <v-col class="text-right">
          <v-btn color="primary" class="mr-2" @click="addDialog = true">
            Añadir Aro <v-icon>mdi-plus</v-icon>
          </v-btn>
        </v-col>
      </v-row>
      <v-row dense class="pa-2 align-center">
...
export default {
  data() {
    return {
      breadcrumbs_title: "Tipos de Aro",
      breadcrumbs: [
      ...
      headers: [
        { text: "#", value: "id_tipo_aro", align: "left" },
        { text: "Tipo de Aro", value: "tipo_aro", align: "center" },
  ...
  methods: {
    ...
    async getRegistros(page = 1, per_page = 25, sortDesc = 0, sortBy = "") {
      ...
      this.data_reg = [];
      try {
        const response = await API.producto_aro.list(
          "?page=" +
            page +
            "&itemsPerPage=" +
            per_page +
    ...
    async deleteRegConfirm() {
      ...
      try {
        const response = await API.producto_aro.remove(
          this.addForm.id_tipo_aro
        );
        UTILS.toastr.success("Aro eliminado correctamente", this);
      ...
      if (this.$refs.addForm.validate()) {
        try {
          ...
          if (this.dialogEditar == false) {
            const response = await API.producto_aro.create(this.addForm);
            UTILS.toastr.success("Aro creado correctamente", this);
          } else {
            const response = await API.producto_aro.update(
              this.addForm.id_tipo_aro,
              this.addForm
            );
            UTILS.toastr.success("Aro actualizado correctamente", this);
          }
          this.getRegistros();
