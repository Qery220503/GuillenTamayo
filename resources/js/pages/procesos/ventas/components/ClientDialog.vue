  ...
  methods: {
    ...
    async open() {
      try {
        let vm = this;
        this.loading = true;
        await vm.getTiposDoc();
        await vm.getOcupacion();
        await vm.getDepartamento();
        await vm.getProvincia(4);
        await vm.getDistrito(401);

        if(vm.doc_type === 1 || vm.doc_type === 2){
          vm.items_tipo_doc = vm.doc_types.filter(e => e.id_tipo_documento === this.doc_type);
        }else{
          vm.items_tipo_doc = vm.doc_types;
        }

        console.log(vm.doc_types);
