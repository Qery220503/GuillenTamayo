        ...
        <v-card>
            ...
            <v-card-text>
                <v-simple-table>
                    <thead>
                        <tr>
                            <th class="text-left">Guía/Factura #</th>
                    ...
                    <tbody>
                        <tr v-for="(item, i) in debtsData" :key="i">
                            <td>{{ item.guia_factura }}</td>
                            <td>{{ item.nombre_proveedor }}</td>
                            <td>S/. {{ item.total }}</td>
                            <td>S/. {{ item.deuda_total_abonado }}</td>
                            <td>S/. {{ item.deuda_saldo }}</td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr>
                            <td colspan="4" style="text-align:right;"><b>Total: </b></td>
                            <td colspan="1"><b>S/. {{ calcularTotalSaldoPendiente() }}</b></td>
                        </tr>
                    </tfoot>
                </v-simple-table>
...
export default {
    ...
    methods:{
        registrarPagoGrupal(){
            const debtsData = this.debtsData.map((e) => e.id_compra);
            let IDs = debtsData.join(",");
            axios.post('/api/'+this.cuentaRoute+'/pagoGrupal/'+IDs, this.formPago).then((response)=>{
