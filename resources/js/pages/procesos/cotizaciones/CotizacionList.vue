    ...
    <div class="d-flex flex-column flex-grow-1">
        ...
        <v-card>
            ...
            <v-row dense class="pa-2 align-center overflow-x-auto ">
                <v-data-table light :headers="headers" :items="data_reg.data" :page="current_page"
                    :items-per-page="itemsperpage" :server-items-length="total_reg" :options.sync="dataTabOptions"
                    class="flex-grow-1 scroll-me" :footer-props="{
                        itemsPerPageOptions: [25, 50, 100, 1000]
                    }" :loading="loadingTable" loading-text="Cargando... Por favor, espere">
                    <template v-slot:body.prepend>
                        <tr>
                            <td>
                                <v-text-field type="text" outlined dense class="mt-1" hide-details
                                    v-model="filter.correlativo" @input="searchCotizaciones()"></v-text-field>
                            </td>
                            <td>
                                <v-text-field type="text" outlined dense class="mt-01" hide-details v-model="filter.cliente"
                                    @input="searchCotizaciones()"></v-text-field>
                            </td>
                            <td>
                                <!--<v-text-field
                  type="date"
                ...
                ></v-text-field>-->
                                ...
                                <v-menu v-model="menu" :close-on-content-click="false" :nudge-right="40"
                                    ...
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-text-field v-model="filter.fecha" outlined dense hide-details v-bind="attrs"
                                            v-on="on" readonly @change="searchCotizaciones()" clearable></v-text-field>
                                    </template>
                                    <v-date-picker v-model="filter.fecha"
                                        @input="menu = false; searchCotizaciones()"></v-date-picker>
                                </v-menu>
                            </td>
                            <td>
                    ...
                    <template v-slot:[`item.cliente`]="{ item }">
                        <div :class="{ indicador: true, indicador_rojo: (item.cliente.lista_negra == 1) }"
                            style="width: 5px; height: 5px; display: inline-block; border-radius: 50%;"></div>
                    ...
                    <template v-slot:[`item.actions`]="{ item }">
                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn color="primary" icon :href="'/cotizacionPDf/' + item.id_cotizacion" target="_blank"
        ...
        <v-dialog v-model="cotizacionesDialog" scrollable :overlay="false" width="85%">
            <v-card>
                <v-card-text>
                    <v-data-table :headers="headersCotizacion" :items="detalle_cotizacion" show-select
                        v-model="seleccionCotizaciones" item-key="id_cotizacion_detalle">
                        <template v-slot:top>
                            <v-toolbar flat>
                                <v-toolbar-title>
                        ...
                        <template v-slot:[`item.actions`]="{ item }">
                            <v-btn icon small color="blue darken-4" class="ml-1" @click="
        ...
        <v-dialog v-model="dialogCliente" @click:outside="resetCotizacion">
            <v-card>
                <v-card-title class="text-h5">
                    Crear Cotización
                    <div :class="{ indicador: true, indicador_rojo: (cliente.lista_negra == 1) }"></div>
                </v-card-title>

                <v-card-text class="px-5">
                    <v-form ref="clientForm" v-model="valid" @submit.prevent="sendCotizacion()">
                        <v-row class="mt-2" style="padding: 0px 10px !important;">
                            <v-col class="py-2 px-1" cols="12" md="3">
                                <v-select :items="items_tipo_doc" label="Tipo Documento"
                                    placeholder="Selecciona un tipo documento" v-model="cliente.cod_tipo_doc"
                                    :rules="[rules.required]" required item-text="nombre_tipo_documento"
                                    item-value="id_tipo_documento" filled dense></v-select>
                            </v-col>
                            <v-col class="py-2 px-1" cols="12" md="3">
                                <v-text-field v-model="cliente.nro_documento" label="Nro. Documento"
                                    :rules="[rules.required, rules.dni]" v-if="cliente.cod_tipo_doc == 1" filled dense
                                ...
                                <v-text-field v-model="cliente.nro_documento" label="Nro. Documento"
                                    :rules="[rules.required, rules.ruc]" v-if="cliente.cod_tipo_doc == 2" filled dense
                                ...
                                <v-text-field v-model="cliente.nro_documento" label="Nro. Documento"
                                    :rules="[rules.required]" v-if="cliente.cod_tipo_doc != 1 && cliente.cod_tipo_doc != 2"
                                    filled dense autocomplete="off" maxlength="11">
                                    <template #append-outer>
                                        <v-btn color="primary" class="mb-1" @click="obtenerDataDocumentos()">
                                            <v-icon>mdi-magnify</v-icon>
                                        </v-btn>
                            ...
                            <v-col class="py-2 px-1" cols="12" md="3">
                                <v-text-field v-model="cliente.nombre_razon_social" label="Nombre" :rules="[rules.required]"
                                    autocomplete="off" filled dense></v-text-field>
                            </v-col>
                            <v-col class="py-2 px-1" cols="12" md="3">
                                <v-menu v-model="menu2" :close-on-content-click="false" transition="scale-transition"
                                    offset-y min-width="auto">
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-text-field v-model="cliente.fecha_nacimiento" label="Fecha Nacimiento"
                                            prepend-inner-icon="mdi-calendar" v-bind="attrs" v-on="on"
                                            :rules="[rules.required]" dense filled></v-text-field>
                                    </template>
                                    <v-date-picker v-model="cliente.fecha_nacimiento" @input="menu2 = false"
                                        no-title></v-date-picker>
                                </v-menu>
                            </v-col>
                            <v-col class="py-1 px-1" cols="12" md="3">
                                <v-text-field v-model="cliente.telefono" label="Teléfono Principal" maxlength="9"
                                    :rules="[rules.required, rules.only_numbers]" filled dense
                                    autocomplete="off"></v-text-field>
                            </v-col>
                            <v-col class="py-1 px-1" cols="12" md="3">
                                <v-text-field v-model="cliente.telefono_adic" label="Teléfono Adicional" maxlength="9"
                                    :rules="[rules.only_numbers]" filled dense autocomplete="off"></v-text-field>
                            </v-col>
                            <v-col class="py-1 px-1" cols="12" md="3">
                                <v-text-field v-model="cliente.email" label="Correo Electrónico" :rules="[rules.email]"
                                    filled dense autocomplete="off" type="email"></v-text-field>
                            </v-col>
                            <v-col class="py-1 px-1" cols="12" md="3">
                                <v-select :items="items_ocupacion" label="Ocupación/Trabajo"
                                    placeholder="Selecciona una ocupación" v-model="cliente.id_ocupacion" filled dense
                                    item-text="nombre_ocupacion" item-value="id_ocupacion"></v-select>
                            </v-col>
                            <v-col class="py-1 px-1" cols="12" md="3">
                                <v-select :items="items_departamento" label="Departamento"
                                    placeholder="Selecciona un departamento" v-model="cliente.id_departamento"
                                    :rules="[rules.required]" filled dense item-text="nombre_departamento"
                                    item-value="id_departamento"></v-select>
                            </v-col>
                            <v-col class="py-1 px-1" cols="12" md="3">
                                <v-select :items="items_provincia" label="Provincia" placeholder="Selecciona una provincia"
                                    v-model="cliente.id_provincia" :rules="[rules.required]" filled dense
                                    item-text="nombre_provincia" item-value="id_provincia"></v-select>
                            </v-col>
                            <v-col class="py-1 px-1" cols="12" md="3">
                                <v-select :items="items_distrito" label="Distrito" placeholder="Selecciona un distrito"
                                    v-model="cliente.id_distrito" :rules="[rules.required]" filled dense
                                    item-text="nombre_distrito" item-value="id_distrito"></v-select>
                            </v-col>
                            <v-col class="py-1 px-1" cols="12" md="3">
                                <v-text-field v-model="cliente.direccion" label="Dirección" :rules="[rules.required]" filled
                        ...
                        <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn color="error" class="ml-2" @click="resetCotizacion">Cancelar</v-btn>
...
export default {
    ...
    methods: {
        ...
        async loadPdfOpcion(id_cotizacion, id_cotizacion_detalle) {
            ...
            let pdfURL =
                "/cotizacionPDf/" + id_cotizacion + "/" + detalle.join(',');
        ...
        generatePDF() {
            ...
            const detalles = this.seleccionCotizaciones.map((e) => e.id_cotizacion_detalle);
