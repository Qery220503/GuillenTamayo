<template>
    <div class="flex-grow-1">
        ...
        <v-tabs v-model="rolesTab" :show-arrows="false" background-color="transparent">
            ...
            <v-tab v-if="$can('permissions_read', 'all')" to="#tabs-permissions"><b>PERMISOS</b></v-tab>
        </v-tabs>
        <br>
        
        <v-tabs-items v-model="rolesTab" id="custom-tab-items">
            <v-tab-item value="tabs-roles">
                <roles-page ref="roles_page" v-if="$can('roles_read', 'all')">    
                </roles-page>
...
export default {
    ...
    data: () => ({
        preloader: false,
        breadcrumbs_title: "Roles y Permisos",
