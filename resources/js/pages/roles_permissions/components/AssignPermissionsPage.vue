<template>
    <div>
        ...
        <v-card class="mb-4" light style="padding: 15px">
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="error" text @click="ClearPermissions">Cancelar</v-btn>
...
export default {
    ...
    methods: {
        ...
        async GetRecords(page = 1, perPage = 25){
            ...
            this.dataReg = [];
            try{
                const response = await API.permissions.list("?page="+page +"&perPage="+perPage
        ...
        async SavePermissions(id){
            this.preloader = true;
            try{
                const response = await API.permissions.assignPermissionsToRole(id, 
        ...
        async GetRolePermissions(id){
            try{
                const response = await API.permissions.show(id);
